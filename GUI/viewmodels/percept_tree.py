...
from GUI.helpers.gui_helpers import insert_blank_row
...
#Tkinter rendering
def render_percept_tree(
    gui,
    nodes,
    parent="",
    npc=None
):
    # render_percept_tree() is currently designed for
    # the NPC Percepts tab, not the Sublocation Inspector.
    ...
    for node in nodes:
        ...
        if (
            node.data.get("display_aggregate")
            and not separator_inserted
        ):
            insert_blank_row(tree, parent)
        ...
        if data.get("display_aggregate"):#there is also this
            ...
            if isinstance(origin, Book):#shoudl we be cautious of turning this function into a giant dispathcer

                name, description, info, highlight = get_book_display_data(#another pattern here?
                    v,
                    npc
                )
